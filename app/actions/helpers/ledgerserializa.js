...
export default function serialize(chainId, transaction, types) {
    ...
    encode(writter, FcBuffer.toBuffer(types.unsigned_int(), transaction.net_usage_words));
    encode(writter, FcBuffer.toBuffer(types.uint8(), transaction.max_cpu_usage_ms));
    encode(writter, FcBuffer.toBuffer(types.unsigned_int(), transaction.delay_sec));
    ...
    encode(writter, FcBuffer.toBuffer(types.unsigned_int(), action.authorization.length));
    for (let i = 0; i < action.authorization.length; i += 1) {
        const authorization = action.authorization[i];

        encode(writter, FcBuffer.toBuffer(types.account_name(), authorization.actor));
